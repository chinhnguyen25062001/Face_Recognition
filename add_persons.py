...
from face_recognition.utils import read_features

# Check if CUDA is available and set the device accordingly
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Initialize the face detector (Choose one of the detectors)
detector = YOLO('face_detection/weights/yolov8n-face.pt')

# Initialize the face recognizer
recognizer = iresnet_inference(
    model_name="r100", path="face_recognition/weights/arcface_r100.pth", device=device
)
    ...
    for name_person in os.listdir(add_persons_dir):
        ...
        for image_name in os.listdir(person_image_path):
            if image_name.endswith(("png", "jpg", "jpeg")):
                ...
                result = output[0]
                bboxes = result.boxes.xyxy.detach().cpu().numpy().tolist()
